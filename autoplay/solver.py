...
from autoplay.domain import Move, apply_move, completed_lines, legal_moves, matched_cell_count, validate_board
...
class Solver:
    ...
    def _cohesion(board: np.ndarray) -> int:
        def line_score(line: np.ndarray) -> int:
            normal = line[(line != 0) & (line != 5)]
            if not normal.size:
                return 0
            counts = np.bincount(normal)
            return int(counts.max() ** 2 + np.count_nonzero(line == 5) * counts.max())
